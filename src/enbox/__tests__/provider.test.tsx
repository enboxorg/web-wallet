...
function InitializationStatus() {
  const { error, retryInitialization } = useEnboxAuth();
  if (!error) return <span>initializing</span>;
...
describe('EnboxAuthProvider restore flow', () => {
  ...
  it('allows initialization to be retried after manager creation fails', async () => {
    const user = userEvent.setup();
  ...
  it('configures AuthManager to own live sync for every session flow', async () => {
    const auth = createAuth();
    authMocks.create.mockResolvedValue(auth);
  ...
  it('does not start restored-session sync a second time in the provider', async () => {
    const user = userEvent.setup();
    const settled = vi.fn();
    const auth = createAuth('locked');
    auth.restoreSession.mockImplementation(async () => {
    ...
    expect(useAuthStore.getState().agent).toBe(auth.agent);
    expect(auth.agent.sync.startSync).not.toHaveBeenCalled();
  });
